...
export function AnalyticsPage({ token }) {
  ...
  useEffect(() => {
    if (!token) return;
    api.fetchAnalytics(token).then(setAnalytics).catch(console.error);
  }, [token]);

  const metrics = [
    { label: 'Fuel efficiency', value: analytics?.analytics?.fuelEfficiency || '6.8', suffix: 'km/l' },
    { label: 'Fleet utilization', value: analytics?.analytics?.fleetUtilization ?? '68', suffix: '%' },
    { label: 'Operational cost', value: money(analytics?.analytics?.operationalCost ?? 352400), suffix: '' },
    { label: 'Vehicle ROI', value: '18.4', suffix: '%' },
  ];
