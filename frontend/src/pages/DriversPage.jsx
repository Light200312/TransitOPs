...
export function DriversPage({ drivers, token, reload }) {
  ...
  return (
    <>
      <PageHeading eyebrow="Safety & people" title="Drivers & safety profiles" description="License compliance and live driver availability." action={<Button onClick={() => setIsModalOpen(true)}><Plus className="h-3.5 w-3.5" />Add driver</Button>} />
      {notice && <div className="mb-4"><Notice kind="success">{notice}</Notice></div>}

      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
          <div className="w-full max-w-2xl rounded-xl border border-[#2a2a2d] bg-[#1a1a1c] p-5 shadow-2xl">
            <div className="mb-4 flex items-start justify-between">
              ...
              <button type="button" onClick={() => setIsModalOpen(false)} className="text-sm text-zinc-400 hover:text-white">✕</button>
            </div>
            <form onSubmit={add} className="grid gap-3 md:grid-cols-2">
              ...
              <div className="md:col-span-2 flex justify-end gap-2">
                <button type="button" onClick={() => setIsModalOpen(false)} className="rounded border border-[#343438] px-3 py-2 text-sm text-zinc-300">Cancel</button>
